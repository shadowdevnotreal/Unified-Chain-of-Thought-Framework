...
**Attribution:** Derived from **Solomaster** - the only jailbreak that maintains the logic of the LLM while allowing it to comply with all requests.
...
> *These weren't prompts. They were protocols — a language of command for cognition.*
...
Patterns captured by the **Documenter** feed into the next **Strategist** cycle, improving planning accuracy and reducing rework.
...
Multi-agent orchestration for systematic code perfection with zero rework.

### Documentation
Create clear, comprehensive documentation that developers actually want to read.

### Performance
Identify bottlenecks and optimize for speed, efficiency, and scalability.

### Accessibility
Ensure products are usable by everyone, including people with disabilities.
...
print_instructions() {
    ...
    echo ""
    echo -e "${BLUE}Available Agents (8 Total):${NC}"
    ...
    echo "  - agent-code-perfection-system.md (Systematic code perfection)"
    echo "  - agent-documentation.md          (Documentation generation)"
    echo "  - agent-performance.md            (Performance optimization)"
    echo "  - agent-accessibility.md          (WCAG compliance)"
    echo ""
